...
import { commentTexts } from '../../data.jsx'

function Comment({ id, username, comment, profileImage, onRemove }) {
  return (
    <div className='comment-container flex flex-col w-full max-w-[1244px]'>
        <div className="comment-header flex flex-row items-start gap-[20px]">
            <div className="pfp flex-shrink-0">
                <img src={profileImage} alt="profile" className='w-[55px] h-[55px] rounded-full' />
            </div>
            <div className="flex flex-col flex-1">
              <div className="comment-body bg-[#F1F2F5] rounded-[0px_20px_20px_20px] px-5 py-3 w-full">
                  <div className="username mb-2">
                      <p className='text-[14px] sm:text-[15px] md:text-[16px] font-[Poppins] font-[500] text-black'>{username}</p>
                  </div>
                  <div className="comment">
                      <p className='text-[14px] sm:text-[15px] md:text-[16px] font-[Poppins] font-[400] text-[#606162]'>
                      {comment}
                      </p>
                  </div>
              </div>
              <div className="remove-comment flex flex-row items-center gap-[10px] mt-2 pl-5 cursor-pointer hover:opacity-70 transition-opacity" onClick={() => onRemove(id)}>
                <svg width="19" height="21" viewBox="0 0 19 21" fill="#666666" color='#666666' xmlns="http://www.w3.org/2000/svg">
                ...
                <p className='text-[14px] sm:text-[15px] md:text-[16px] font-[Poppins] font-[400] text-[#666666]'>{commentTexts.removeComment}</p>
              </div>
            </div>
        </div>
    </div>
